...
fn subtract_nfps(subject_nfps: Vec<Vec<f32>>, clip_nfps: Vec<Vec<f32>>) -> Vec<Vec<f32>> {
    if subject_nfps.is_empty() || clip_nfps.is_empty() {
        return subject_nfps;
    }

    let subjects = subject_nfps
        .iter()
        .map(|mem_seg| clipper_utils::from_mem_seg(mem_seg, None, false))
        .collect::<Vec<_>>();
    let clips = clip_nfps
        .iter()
        .map(|mem_seg| clipper_utils::from_mem_seg(mem_seg, None, false))
    ...
    clipper.add_paths(&subjects, PolyType::Subject);
    clipper.add_paths(&clips, PolyType::Clip);
    ...
    clipper_utils::clean_polygons(&result, 0.1)
        .into_iter()
        .map(|polygon| clipper_utils::to_mem_seg(&polygon))
        .filter(|mem_seg| mem_seg.len() >= 6)
        .collect()
}
...
pub unsafe fn pair_data(buffer: &[f32]) -> Vec<f32> {
    ...
    if pair_content.is_inside() {
        ...
        if pair_content.use_holes() {
            ...
            let mut blocked_nfps = Vec::new();

            for node in children.iter() {
                bind_polygon(&mut child, node);

                let mut no_fit_polygons = pair_outside(
                    &mut pool,
                    &mut scan_polygon,
                    &mut child,
                    &mut polygon_b,
                ...
                blocked_nfps.append(&mut no_fit_polygons);
            }

            nfp = subtract_nfps(nfp, blocked_nfps);
        }
