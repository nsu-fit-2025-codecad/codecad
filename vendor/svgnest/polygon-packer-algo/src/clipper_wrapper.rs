...
/// Offset a polygon node while preserving its current coordinate space.
///
/// # Arguments
/// * `mem_seg` - Flat array of f32 values representing points (x0, y0, x1, y1, ...)
...
///
/// # Returns
/// Offset polygon as a flat `Vec<f32>`, or the input if spacing is 0.
...
fn translate_mem_seg(mem_seg: &mut [f32], dx: f32, dy: f32) {
...
fn translate_node(node: &mut PolygonNode, dx: f32, dy: f32) {
    translate_mem_seg(&mut node.mem_seg, dx, dy);
...
fn reset_node_tree_position(node: &mut PolygonNode) {
    if node.mem_seg.len() < 6 {
        return;
    }

    let point_count = node.mem_seg.len() >> 1;
    let bounds = f32::calculate_bounds(&node.mem_seg, 0, point_count);
    translate_node(node, -bounds[0], -bounds[1]);
}
...
fn reset_bin_tree_position(node: &mut PolygonNode) {
    reset_node_tree_position(node);
}
...
/// Generate bounds for a bin polygon with explicit forbidden holes.
/// Returns: (bounds, result_bounds, area, node)
pub fn generate_bounds_with_holes(
    mem_seg: &[f32],
    holes: &[Vec<f32>],
    spacing: i32,
    curve_tolerance: f64,
    ...
    for hole in holes {
        ...
        let mut hole_node = PolygonNode::new(-2, 0.0, hole.clone());
        clean_node(&mut hole_node, curve_tolerance);
        offset_node(&mut hole_node, 1, spacing, curve_tolerance);
        bin_node.children.push(hole_node);
    }

    reset_bin_tree_position(&mut bin_node);
    simplify_nodes(std::slice::from_mut(&mut bin_node));
    ...
    let mut area = f32::polygon_area(&bin_node.mem_seg);
    for child in &bin_node.children {
        area -= f32::abs_polygon_area(&child.mem_seg);
    }
